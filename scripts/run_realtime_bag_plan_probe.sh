...
planner_max_snap_distance_m="${TGW_PLANNER_MAX_SNAP_DISTANCE_M:-0.75}"
planner_require_footprint="${TGW_PLANNER_REQUIRE_FOOTPRINT:-false}"
...
setsid ros2 launch tgw_planner realtime_mapping.launch.py \
  ...
  validation_require_footprint:="${validation_require_footprint}" >"${log_dir}/tgw.log" 2>&1 &
pids="${pids} $!"
...
class Probe(Node):
    def __init__(self):
        ...
        self.cloud = None
        self.probe_cloud_topic = os.environ.get("TGW_PROBE_CLOUD_TOPIC", "/tgw_map/traversable_cloud")
        self.create_subscription(PointCloud2, self.probe_cloud_topic, self.on_cloud, qos)
...
def main():
    ...
    print(
        ...
        f"top_components:{top_component_limit} component_pool:{component_pool} "
        f"max_plan_attempts:{max_plan_attempts} "
        f"probe_cloud_topic:{os.environ.get('TGW_PROBE_CLOUD_TOPIC', '/tgw_map/traversable_cloud')}")
    ...
    print(f"probe_cloud_points={len(points)}")
    if len(points) < 2:
    ...
    if component_pool == "all":
        candidate_cells = [cell for component in components for cell in component]
    else:
        candidate_cells = components[0] if components else []
    sampled = candidate_cells[::max(1, len(candidate_cells) // max(1, sample_limit))]
    candidate_pairs = []
    target_dxy = 0.5 * (min_dxy + max_dxy)
    for i, a_cell in enumerate(sampled):
        a = cells[a_cell]
        for b_cell in sampled[i + 1:]:
            b = cells[b_cell]
            dz = abs(a[2] - b[2])
            dxy = math.hypot(a[0] - b[0], a[1] - b[1])
            if min_abs_dz <= dz <= max_abs_dz and min_dxy <= dxy <= max_dxy:
                candidate_pairs.append((abs(dxy - target_dxy), a, b, dxy))
    ...
    attempted = 0
    for _, start, goal, dxy in candidate_pairs[:max(1, max_plan_attempts)]:
        attempted += 1
        print(f"attempt={attempted} start={start} goal={goal} dxy={dxy:.3f}")
        ...
        if not future.done() or future.result() is None:
            print(f"attempt={attempted} success=False reason=plan_timeout")
            continue

        response = future.result()
        stats = response.stats
        print(
            f"attempt={attempted} success={response.success} message=\"{response.message}\" "
    ...
    print(f"success=False reason=no_valid_plan_after_attempts attempts={attempted}")
    return 1
